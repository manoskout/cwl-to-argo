...
def extract_file(file_path, temp_folder):
    ...
    try:
        ...
        # TODO --> Delete this exception the file should 
        # be removed if the parsing have been achieved
...
class Workflow:
    ...
    def __init__(self, compressed_workflow_path=None, input_yml=None):
        ...
        if os.path.exists(compressed_workflow_path):
            # print("File found")
            list_of_files, extracted_wf_path = extract_file(
                compressed_workflow_path, "tmp_folder")
            # print(list_of_files, extracted_wf_path)
        else:
            raise CWL_ArgoParserException("File: {compressed_workflow_path} does not exist".format(
                compressed_workflow_path=compressed_workflow_path))
    ...
    def get_wf_bash_files(self, step):
        ...
        '''
        list_of_files = self.parse_step(
            step)['requirements']['InitialWorkDirRequirement']['listing']
    ...
    def get_step_bash_contents(self, step, step_files):
        ...
        if not self.is_cmd_tool(self.parse_step(step)["class"]):
            raise CWL_ArgoParserException(
                "Step {step} is not contains bash file".format(step=step))
    ...
    def parse_workflow(self):
        ...
        if "workflow.cwl" in self.workflow_files:
            workflow_file_path = os.path.join(
                self.extracted_wf_path, "workflow.cwl")
...
if __name__ == '__main__':
    ...
    workflow = Workflow(
        compressed_workflow_path=args.workflow_filename, input_yml=args.input_file)
